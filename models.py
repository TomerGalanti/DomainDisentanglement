...
class InputConnector(nn.Module):
    def __init__(self, size):
        super(InputConnector, self).__init__()
        self.layer1 = []

        self.layer1.append(nn.Linear(3 * 128 * 128, size))
        # self.layer1.append(SpectralNorm(nn.Conv2d(3, size, 4, 2, 1)))
        # self.layer1.append(nn.InstanceNorm2d(size))
...
class E_common(nn.Module):
    def __init__(self, sep, size, dim=512):
        ...
        self.layer5.append(SpectralNorm(nn.Conv2d(256, (self.dim - 2 * self.sep), 4, 2, 1)))
        self.layer5.append(nn.InstanceNorm2d((self.dim - 2 * self.sep)))
    ...
    def forward(self, net):
        ...
        out = self.l5(out)
        # out = self.l6(out)
        # input_connector_out = self.input_connector(net.view(-1, 3*128*128))
        out = out.view(-1, self.z_dim_size)
        # out = torch.cat([out, input_connector_out], dim=1)
...
class E_separate_A(nn.Module):
    def __init__(self, sep, size):
        ...
        self.layer5.append(SpectralNorm(nn.Conv2d(256, self.sep, 4, 2, 1)))
        self.layer5.append(nn.InstanceNorm2d(self.sep))
        self.layer5.append(nn.LeakyReLU(0.2, inplace=True))
        self.l5 = nn.Sequential(*self.layer5)

        # self.layer4.append(SpectralNorm(nn.Conv2d(128, 128, 4, 2, 1)))
        # self.layer4.append(nn.InstanceNorm2d(128))
        # self.layer4.append(nn.LeakyReLU(0.2, inplace=True))
        # self.l4 = nn.Sequential(*self.layer4)
        #
        # self.layer5.append(SpectralNorm(nn.Conv2d(128, 128, 4, 2, 1)))
        # self.layer5.append(nn.InstanceNorm2d(128))
        # self.layer5.append(nn.LeakyReLU(0.2, inplace=True))
        # self.l5 = nn.Sequential(*self.layer5)
        #
        # self.layer6.append(SpectralNorm(nn.Conv2d(128, self.sep, 4, 2, 1)))
        # self.layer6.append(nn.InstanceNorm2d(self.sep))
        # self.layer6.append(nn.LeakyReLU(0.2, inplace=True))
        # self.l6 = nn.Sequential(*self.layer6)
...
class Decoder(nn.Module):
    def __init__(self, size, dim=512):
        ...
        self.layer1.append(SpectralNorm(nn.ConvTranspose2d(self.dim, 256, 4, 2, 1)))
        self.layer1.append(nn.InstanceNorm2d(256))
        self.layer1.append(nn.ReLU(inplace=True))
        self.l1 = nn.Sequential(*self.layer1)

        self.layer2.append(SpectralNorm(nn.ConvTranspose2d(256, 128, 4, 2, 1)))
        self.layer2.append(nn.InstanceNorm2d(128))
        self.layer2.append(nn.ReLU(inplace=True))
        self.l2 = nn.Sequential(*self.layer2)

        self.layer3.append(SpectralNorm(nn.ConvTranspose2d(128, 64, 4, 2, 1)))
        self.layer3.append(nn.InstanceNorm2d(64))
        self.layer3.append(nn.ReLU(inplace=True))
        self.l3 = nn.Sequential(*self.layer3)

        self.layer4.append(SpectralNorm(nn.ConvTranspose2d(64, 32, 4, 2, 1)))
        self.layer4.append(nn.InstanceNorm2d(32))
        self.layer4.append(nn.ReLU(inplace=True))
        self.l4 = nn.Sequential(*self.layer4)

        self.layer5.append(nn.ConvTranspose2d(32, 3, 4, 2, 1))
        self.layer5.append(nn.Tanh())
        self.l5 = nn.Sequential(*self.layer5)
...
class Classifier2(nn.Module):
    def __init__(self, sep, size):
        ...
        self.fc1 = nn.Linear((512 - 2 * self.sep) * self.size * self.size, 120)
...
class Classifier(nn.Module):
    def __init__(self, common_dim, sepA_dim, sepB_dim):
        super(Classifier, self).__init__()

        self.common_dim = common_dim
        self.sepA_dim = sepA_dim
        self.sepB_dim = sepB_dim
        self.input_dim = common_dim + sepA_dim + sepB_dim
